...
import { useMyFulfilledRequests } from "./hooks/useRequests";
import { RequestStatus } from "./types/request.types";
import { RequestCard } from "./components/RequestCard";

const MyFulfilledRequestsPage = () => {
  const { t } = useTranslation();
  const [statusFilter, setStatusFilter] = useState<string>("all");

  // Fetch my fulfilled requests
  const { data, isLoading } = useMyFulfilledRequests();
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box sx={{ mb: 4 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 1 }}>
          <CardGiftcard sx={{ fontSize: 40, color: "primary.main" }} />
          <Typography variant="h4" fontWeight={700}>
            {t("myFulfilledRequests.title") || "My Fulfilled Requests"}
          </Typography>
        </Box>
        <Typography variant="body1" color="text.secondary">
          {t("myFulfilledRequests.subtitle") || "Track requests you've fulfilled"}
        </Typography>
      </Box>
      ...
      {!isLoading && filteredRequests.length === 0 && (
        ...
        >
          ...
          <Typography variant="h6" color="text.secondary" gutterBottom>
            {t("myFulfilledRequests.noRequests") || "No fulfilled requests found"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("myFulfilledRequests.noRequestsDescription") || "Requests you fulfill will appear here"}
          </Typography>
        </Box>
      )}
...
export default MyFulfilledRequestsPage;
